...
export default class CreateRide extends React.Component<{}, RideDetails> {
            ...
            if (sessionStorage.getItem('carDetails') === null) {
                ...
                RideService.getRideById(sessionStorage.getItem('rideId')).then((response) => {
                    if (response !== undefined) {
                        this.setState({ data: { ...this.state.data, from: response[0].From, to: response[0].To, date: (response[0].TravelDate).split('T')[0], time: response[0].Time } });
    ...
    isValidTo(value: any) {
        ...
        this.setState({ meta: {...this.state.meta,toError:isEmpty ? Required : (isValid ? 'Source and destination is different' : '') }})
                ...
                <Grid className='create-ride' item md={4}>
                    <form className='ride-details'>
                        ...
                        <div className='input-box'>
                            <Autocomplete freeSolo options={CityService.getValidCity(this.state.data.from).map((option) => option.Title)} onChange={(event: any, newInputvalue: any) => {this.onSelect(newInputvalue, 'from') }} value={this.state.data.from} renderInput={(param) => (
                        ...
                        <div className='input-box'>
                            <Autocomplete freeSolo options={CityService.getValidCity(this.state.data.to).map((option) => option.Title)} onChange={(event: any, newInputvalue: any) => { this.onSelect(newInputvalue, 'to') }} value={this.state.data.to} renderInput={(param) => (
