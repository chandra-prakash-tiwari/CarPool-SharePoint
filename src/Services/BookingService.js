...
function addBookings(booking, noofSeats) {
  ...
  var url =
    "https://chandraprakashtiwariv.sharepoint.com/sites/carpool/_api/lists/getbytitle('Bookings')/items";
  return fetch(url, {
    method: "POST",
    headers: {
...
function searchRide(bookingSearch) {
  var url =
    "https://chandraprakashtiwariv.sharepoint.com/sites/carpool/_api/lists/getbytitle('Rides')/items?";
  var query = `$filter=(ViaPointsId eq '${
    CityService.getValidCity(bookingSearch.from)[0].ID
  }') and (ViaPointsId eq '${
    CityService.getValidCity(bookingSearch.to)[0].ID
  }') and (TravelDate eq '${bookingSearch.date}')`;
    ...
    .then(async (response) => {
      if (response.status === 200) {
        ...
        for (var i = 0; i < data.value.length; i++) {
          var data1 = [];
          if (
            data.value[i].ViaPointsId.indexOf(
              CityService.getValidCity(bookingSearch.to)[0].ID
            ) >
            data.value[i].ViaPointsId.indexOf(
              CityService.getValidCity(bookingSearch.from)[0].ID
...
function myBookings() {
  ...
  return fetch(url + query, {
    ...
    headers: {
      Accept: "application/json;odata=nometadata",
      "Content-Type": "application/json",
      Authorization: `Bearer ${UserService.getCookie}`,
    },
  })
    .then(async (response) => {
      if (response.status === 200) {
